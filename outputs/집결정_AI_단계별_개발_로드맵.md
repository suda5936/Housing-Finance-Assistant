...
| AI               | 교체 가능한 LLM 어댑터              | 구조화 추출, 추가 질문, 근거 기반 설명            |
...
- [ ] 빈 화면부터 API 헬스체크까지 로컬에서 실행된다.
- [ ] lint·test·typecheck를 각각 실행할 수 있다.
- [ ] 외부 공급자는 어댑터 인터페이스 뒤에 격리되어 있다.
- [ ] OCR·LLM 없이도 수동 입력 데모 경로가 설계되어 있다.
...
### 7.3 개발 시작 전 반드시 확정할 미결정 사항

다음은 구현 중 임의로 결정하면 결과가 흔들리므로 Phase 0에서 반드시 확정해야 한다.

1. MVP에 포함할 실제 정책 3~5개와 검토 담당자
2. 기준소득을 세후 월소득으로 사용할지 여부
3. 보증금 자기자금의 기회비용 포함 여부와 기본값
4. 통근시간·교통비 데이터 공급자 또는 수동 입력 폴백
5. 관리비 미상·범위형 값의 기본 시나리오
6. 후보 평가 기준과 기본 가중치의 근거
7. 원본 문서의 기본 보관기간과 즉시 삭제 옵션
8. 사용할 OCR·LLM 공급자와 데이터 처리 조건
9. 정책 규칙을 검토할 금융·주거 분야 담당자
10. 출시 품질 목표와 경진대회까지의 실제 일정·인력
...
실제 개발은 **Phase 0의 미결정 사항 확정 → Phase 1 기반 구축 → Phase 2 데이터 모델 → Phase 3 계산 엔진 → Phase 4 정책 규칙 엔진** 순으로 시작한다. 이 다섯 단계가 안정되기 전에는 LLM 중심의 에이전트 구현을 앞당기지 않는다.
