...
- [x] 동일 입력·버전으로 동일 판정과 계산을 재현할 수 있다.
- [x] 데이터 삭제가 자동 테스트로 검증된다.
- [x] 원본 문서 접근이 최소 권한으로 제한된다.
- [x] 합성 데이터만으로 로컬 자동 테스트가 가능하다.

### Phase 2 완료 기록

Phase 2의 ERD, API·DB 스키마, 익명 세션 토큰, 개인정보 분류, 입력 스냅샷, 보관기한·삭제 흐름과 테스트 결과는 [Phase 2 데이터 모델·개인정보 기반](./Phase_2_데이터_모델_개인정보_기반.md)에 기록했다.
...
- [x] 모든 공식에 단위 테스트가 있다.
- [x] 동일 입력은 항상 동일 결과를 낸다.
- [x] 계산 중간값과 가정을 사용자에게 설명할 수 있다.
- [x] 누락값이 있는 경우 임의 추정과 판정 보류를 구분한다.
- [x] 스프레드시트 수기 계산 표본과 결과가 일치한다.
- [x] 지도 API 없이 수동 통근시간·교통비 입력으로 동일 계산을 완료한다.
- [x] 주소·통근 데이터의 기준과 조회 시각이 결과에 표시된다.

### Phase 3 완료 기록

Phase 3의 계산 공식, 시나리오 규칙, 입력·출력 스키마, 수기 대조 표본, 통근 수동 폴백과 테스트 결과는 [Phase 3 주거비 계산 엔진](./Phase_3_주거비_계산_엔진.md)에 기록했다.
...
- [x] 정책별 경계값 테스트가 있다.
- [x] 규칙 판정은 LLM 호출 없이 수행된다.
- [x] 모든 결과에 사용한 규칙 버전과 출처가 남는다.
- [x] 정보부족과 부적격이 명확히 분리된다.
- [x] 정책 원문 변경 시 재검토 대상 규칙을 찾을 수 있다.

### Phase 4 완료 기록

Phase 4의 정책 레지스트리, 규칙 스키마, 5상태 사전자격 평가기, 출처·버전 추적, 변경 영향 식별, 계약 전 체크리스트와 승인·롤백 절차는 [Phase 4 정책 데이터·규칙 엔진](./Phase_4_정책_데이터_규칙_엔진.md)에 기록했다. 기술 구현은 완료했지만 실제 정책 활성화는 원문 파일 SHA-256 등록과 팀원 독립 검토가 끝날 때까지 승인 게이트로 차단한다.
...
- [x] 동일 입력과 가중치로 동일 순위가 재현된다.
- [x] 특정 기준 하나가 임의로 순위를 지배하지 않는지 검토되었다.
- [x] 결측값이 많은 후보는 사용자에게 명확히 표시된다.
- [x] 순위가 바뀌는 조건을 수치로 보여줄 수 있다.
- [x] 계산 결과와 자연어 설명을 별도로 검증할 수 있다.
- [x] 생활 인프라 점수의 원천과 산식 또는 사용자 입력 여부를 확인할 수 있다.

### Phase 5 완료 기록

Phase 5의 하드 제약, 가중점수, min-max 정규화, 결측값·동률·압도적 열위 처리, 기준별 기여도, 트레이드오프, ±가중치 민감도와 출처 추적은 [Phase 5 후보 비교·다기준 최적화](./Phase_5_후보_비교_다기준_최적화.md)에 기록했다.
...
- [x] 핵심 필드별 정확도를 별도로 측정한다.
- [x] 잘못된 금액·단위가 자동 검증에 걸린다.
- [x] 사용자가 확인하지 않은 추출값은 최종 계산에 사용되지 않거나 명시적으로 표시된다.
- [x] 문서 프롬프트 인젝션 테스트가 통과한다.
- [x] 업로드 실패 시 수동 입력으로 계속 진행할 수 있다.

### Phase 6 완료 기록

Phase 6의 업로드 제한, MIME·파일 구조 검사, 로컬 PDF·OCR 어댑터, 수동 폴백, 개인정보 마스킹, 좌표형 원문 근거, 규칙 기반 필드 추출, 사용자 수정·확정과 합성 평가셋은 [Phase 6 문서 업로드·OCR·구조화 추출](./Phase_6_문서_업로드_OCR_구조화_추출.md)에 기록했다.
...
- [x] 답변의 정책 주장마다 근거 문서와 인용 구간이 있다.
- [x] 만료 정책이 현재 정책처럼 추천되지 않는다.
- [x] 검색 실패가 무근거 생성으로 이어지지 않는다.
- [x] 검색 정확도와 인용 정합성을 별도로 측정한다.
- [x] 규칙 버전과 정책 원문 버전의 연결이 유지된다.

### Phase 7 완료 기록

Phase 7의 공식 출처 허용 목록, 스냅샷 무결성 검증, 의미 단위 청킹, 로컬 하이브리드 검색, 기간·지역 필터, 문장별 인용, 규칙 조건별 근거 연결과 검색 평가 결과는 [Phase 7 정책 RAG·근거 관리](./Phase_7_정책_RAG_근거_관리.md)에 기록했다. 세 정책은 팀원 교차검토가 끝날 때까지 계속 `OFFICIAL_CHECK_NEEDED` 상태를 반환한다.
...
- [x] 정상 흐름과 모든 실패 분기가 테스트된다.
- [x] LLM이 규칙·계산 결과를 임의로 변경할 수 없다.
- [x] 세션 재개 시 동일 상태에서 계속 진행할 수 있다.
- [x] 검증 실패 결과가 사용자에게 최종 추천으로 노출되지 않는다.
- [x] OCR 또는 LLM 장애 시 수동 경로가 작동한다.
- [x] 계약 전 체크리스트의 모든 항목에 버전과 근거가 있다.

### Phase 8 완료 기록

Phase 8의 상태 머신, 누락 질문 우선순위, 사용자 수정값 우선 처리, 도구 캐시와 실행 한도, 실패 폴백, 검증 게이트, 감사 로그, 버전형 계약 전 체크리스트는 [Phase 8 에이전트 오케스트레이션](./Phase_8_에이전트_오케스트레이션.md)에 기록했다. 정책·출처 교차검토 전 실행은 안전하게 `official_check`에서 종료한다.
...
- [x] 사용자가 모든 계산 가정과 출처에 접근할 수 있다.
- [x] 후보 수정·삭제와 조건 재계산이 가능하다.
- [x] 접근성 기본 구조와 자동 코드 검사를 통과한다.
- [x] 오류 메시지가 복구 방법을 구체적으로 안내한다.

### Phase 9 구현 기록

Phase 9의 6단계 반응형 화면, 실제 API 흐름, 후보 수정·삭제, 문서 추출값 대조, 의사결정 카드, 조건 재계산, 근거·가정 상세, 다운로드·삭제와 접근성 기반은 [Phase 9 Web UI·의사결정 카드](./Phase_9_Web_UI_의사결정_카드.md)에 기록했다. 실제 390px 모바일 브라우저 전체 흐름은 Codex 브라우저 런타임의 로컬 권한 오류로 사용자 수동 확인을 기다린다.
...
- [x] 위협 모델과 주요 공격 시나리오가 문서화되었다.
- [x] 개인정보 삭제와 로그 마스킹 테스트가 통과한다.
- [x] 문서·검색 프롬프트 인젝션 테스트가 통과한다.
- [x] 외부 공급자 장애 시 서비스가 안전하게 축소 동작한다.
- [x] 요청 단위로 결과 생성 과정을 추적할 수 있다.

### Phase 10 완료 기록

Phase 10의 위협 모델, 상관관계 ID, 보안 헤더, Host·요청 크기 방어, 비식별 집계 지표, 로그 마스킹 확대, 원격 Ollama 차단, 만료 정리 루프와 운영 전 필수 통제는 [Phase 10 안전성·보안·관측성](./Phase_10_안전성_보안_관측성.md)에 기록했다. TLS·암호화 키·악성코드 스캐너·RBAC·중앙 모니터링은 배포 플랫폼 확정 후 적용해야 한다.
...
- [x] 필수 백엔드 사용자 흐름과 실패 분기의 E2E 테스트가 통과한다.
- [ ] 사람 확인을 포함한 출시 차단 이슈가 0건이다.
- [x] 알려진 한계가 UI와 운영 문서에 반영된다.
- [x] 고정 합성 데이터와 평가 명령으로 결과를 동일하게 재현할 수 있다.

### Phase 11 구현 기록

Phase 11의 버전 고정 골든·경계·공격 데이터셋, 결정론적 출시 평가 하네스, 영역별 지표, 변조 탐지, 장애 복구·레드팀 결과와 수동 출시 게이트는 [Phase 11 통합 테스트·평가·레드팀](./Phase_11_통합_테스트_평가_레드팀.md)에 기록했다. 자동 차단 이슈는 0건이지만 모바일 수동검증과 정책 독립 검토가 남아 `release_ready=false`다.
...
- [x] 외부 공급자 장애 상황에서도 핵심 계산·비교가 작동한다.
- [ ] 운영 대시보드와 오류 알림이 동작한다.

### Phase 12 구현 기록

Phase 12의 환경 검사, 생존·준비 상태 분리, 오프라인 합성 데모 3세트, 배포 승인·롤백·장애 대응, 관측 기준, 3·5·10분 시연과 기술 Q&A는 [Phase 12 배포·시연·운영 준비](./Phase_12_배포_시연_운영_준비.md)에 기록했다. 로컬 데모 자동 검증은 완료했지만 다른 PC 재현, 스테이징 배포·복구와 중앙 알림은 실제 외부 환경 작업이므로 미완료다.
